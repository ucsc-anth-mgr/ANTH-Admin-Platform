...
const SETUP_SCHEMA = {
  ...
  LETTER_WRITERS: {
    tab: 'LetterWriters',
    // Academic Personnel — external letter writers, one row per writer per
    // case (promotion and above-scale files). Stage per CONFIG's
    // LETTER_STAGES; NotesLog is an APPEND-ONLY trail (timestamped, with
    // author) that also receives automatic entries on stage changes, so the
    // follow-up history reads chronologically and nothing is overwritten.
    // EvidenceLinks is JSON: [{name, url, fileId, uploadedAt}] pointing at
    // acceptance-email PDFs stored in the module's Drive folder under
    // LetterWriters/<year>/<candidate>/. The letters themselves live in a
    // separate campus system — never uploaded here.
    // Rank = the writer's title (Professor, Chancellor's Professor…); kept
    // under its original column name for data continuity — the UI says
    // "Title". Discipline = department/field (Anthropology, Archaeology…).
    // Affiliation = the institution.
    headers: ['WriterID', 'CaseID', 'Name', 'Rank', 'Discipline', 'Affiliation', 'Email', 'Homepage',
              'SuggestedBy', 'Stage', 'NotesLog', 'EvidenceLinks',
              'CreatedAt', 'CreatedBy', 'UpdatedAt', 'UpdatedBy'],
    seed: [],
  },
  COMMUNICATIONS_LOG: {
...
function _schemaPlacement() {
  return [
    ...
    { sheetKey: 'GRAD',         def: SETUP_SCHEMA.GRAD_PROGRESS },
    { sheetKey: 'GRAD',         def: SETUP_SCHEMA.GRAD_STATUS },
    { sheetKey: 'GRAD',         def: SETUP_SCHEMA.GRAD_FORMS_SETTINGS },
